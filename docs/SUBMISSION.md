# KijaniKiosk capstone — submission package

**Student:** Brian Kagai Macharia  
**Track:** B (Serverless-first)  
**Release:** [v1.0.0](https://github.com/MachariaBrian12/kijani-kiosk/releases/tag/v1.0.0)

---

## Primary submission URL

**GitHub repository:** https://github.com/MachariaBrian12/kijani-kiosk

---

## Deliverables checklist

| # | Deliverable | Link / file |
|---|-------------|-------------|
| 1 | Project scope (PDF) | [docs/project-scope.pdf](https://github.com/MachariaBrian12/kijani-kiosk/blob/main/docs/project-scope.pdf) |
| 1b | Architecture diagram (PNG) | [docs/architecture.png](https://github.com/MachariaBrian12/kijani-kiosk/blob/main/docs/architecture.png) |
| 2 | Working repository | https://github.com/MachariaBrian12/kijani-kiosk |
| 3 | Pipeline demo | _Add Loom URL or screenshots in LMS_ → see [demo-pipeline-screenshots.md](demo-pipeline-screenshots.md) |
| 4 | Peer feedback log | [docs/peer-feedback-log.md](https://github.com/MachariaBrian12/kijani-kiosk/blob/main/docs/peer-feedback-log.md) · [Issues](https://github.com/MachariaBrian12/kijani-kiosk/issues) |
| 5 | Slide deck (PDF) | [docs/slides.pdf](https://github.com/MachariaBrian12/kijani-kiosk/blob/main/docs/slides.pdf) |
| 6 | Reflection (PDF) | [docs/reflection.pdf](https://github.com/MachariaBrian12/kijani-kiosk/blob/main/docs/reflection.pdf) |

---

## Supporting evidence

| Item | Link |
|------|------|
| Pull request #1 (serverless core) | https://github.com/MachariaBrian12/kijani-kiosk/pull/1 |
| Pull request #2 (capstone offline) | https://github.com/MachariaBrian12/kijani-kiosk/pull/2 |
| AI governance log | [docs/ai-governance-log.md](https://github.com/MachariaBrian12/kijani-kiosk/blob/main/docs/ai-governance-log.md) |
| Governance checklist | [docs/governance-checklist.md](https://github.com/MachariaBrian12/kijani-kiosk/blob/main/docs/governance-checklist.md) |
| Offline validation | `scripts/validate-all.sh` |
| Jenkins pipeline | [Jenkinsfile](https://github.com/MachariaBrian12/kijani-kiosk/blob/main/Jenkinsfile) |

---

## LMS paste block

```
Repository: https://github.com/MachariaBrian12/kijani-kiosk
Release tag: https://github.com/MachariaBrian12/kijani-kiosk/releases/tag/v1.0.0
Scope PDF: https://github.com/MachariaBrian12/kijani-kiosk/blob/main/docs/project-scope.pdf
Architecture PNG: https://github.com/MachariaBrian12/kijani-kiosk/blob/main/docs/architecture.png
Slides PDF: https://github.com/MachariaBrian12/kijani-kiosk/blob/main/docs/slides.pdf
Reflection PDF: https://github.com/MachariaBrian12/kijani-kiosk/blob/main/docs/reflection.pdf
Peer feedback: https://github.com/MachariaBrian12/kijani-kiosk/blob/main/docs/peer-feedback-log.md
Pipeline demo: [PASTE LOOM URL]
```
